...
HARS_INTRO = {
    ...
    'ru': 'Ниже список фраз описывающих определенные чувства, которые испытывают люди. Оцените пациента выбрав ответ '
          'который лучше всего описывает степень в которой он/она находится.'
}
...
MADRS_INTRO = {
    ...
    'ru': 'Обратите внимание: этот тест обычно заполняется медицинским специалистом, он предоставляется здесь '
          ...
          'операется на объективную оценку.\n'
          'Эти вопросы касаются того как пациент чувствовал на протяжении последней недели. Оцените каждый пункт по '
          'шкале от 0 до 6, где 0 означает "нормально/отсутствует" и 6 означает "крайняя степень".\n'
          'Числа 1, 3 и 5 не имеют текстового описания, эти числа представляют уровень между 2мя вариантами.'
}
